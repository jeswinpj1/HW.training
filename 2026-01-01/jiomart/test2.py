...
# OUTPUT_JL = "jiomart_tea_coffee_variants.jsonl"
...
#         # If 'data' is a list of variants, pick first variant
...
#             return {
#                 "regular_price": d.get("mrp"),
#                 "selling_price": d.get("selling_price"),
#                 "discount_percentage": d.get("discount_pct"),
#             }
#         return {}
#     except Exception as e:
#         print("Price API error for", unique_id, e)
#         return {}
...
# with open(OUTPUT_JL, "w", encoding="utf-8") as f:
...
#             unique_id = variant.get("id")  # usually variant ID works for price API
...
#                 "avg_selling_price": variant.get("attributes", {}).get("avg_selling_price", {}).get("numbers", []),
#                 "avg_discount_pct": variant.get("attributes", {}).get("avg_discount_pct", {}).get("numbers", []),
#                 # ---------------- enrich from PDP & Price API ----------------
#                 **extract_pdp_data(pdp_url),
#                 **extract_price_data(unique_id),
#                 "pdp_fetched_at": time.strftime("%Y-%m-%d %H:%M:%S"),
#             }

#             # Save to JSONL
#             with open(OUTPUT_JL, "a", encoding="utf-8") as f:
#                 json.dump(enriched_product, f, ensure_ascii=False)
#                 f.write("\n")
...
# ---------------- CONFIG ---------------- #
BASE_SEARCH_URL = "https://www.jiomart.com/trex/search"
OUTPUT_JL = "06_jiomart_tea_coffee_products.jsonl"
PAGE_SIZE = 50
...
json_data = {
    ...
    "variantRollupKeys": ["variantId"],
    "branch": "projects/sr-project-jiomart-jfront-prod/locations/global/catalogs/default_catalog/branches/0",
    "pageCategories": ["29009"],  # Tea & Coffee
    "userInfo": {"userId": None},
    "pageToken": "",
    "orderBy": "attributes.popularity desc",
    "filter": (
        'attributes.status:ANY("active") AND attributes.category_ids:ANY("29009")'
    ),
    "visitorId": "anonymous-16b88074-4641-4f8e-b5f9-c4e9141e3536",
}
