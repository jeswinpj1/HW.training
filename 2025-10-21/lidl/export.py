...
from settings import FILE_NAME_FULLDUMP, CSV_DELIMITER, CSV_QUOTECHAR, MONGO_URI, MONGO_DB, MONGO_COLLECTION_ENRICHED_DATA
...
col_enriched = mongo_client[MONGO_DB][MONGO_COLLECTION_ENRICHED_DATA]
CSV_HEADERS = [
...
class Export:
    ...
    def generate_product_unique_key(self, doc):
        return f"{doc.get('unique_id','')}P" if doc.get("unique_id") else ""

    def generate_netcontent(self, doc):
        qty, unit = doc.get("grammage_quantity"), doc.get("grammage_unit")
        return f"{qty} {unit}".strip() if qty and unit else ""

    def format_instock(self, doc):
        stock = str(doc.get("instock", "")).strip()
        if stock and "in store from" not in stock.lower():
            return "True"
        return "False"
    ...
    def start(self):
        with open(self.out_path, "w", encoding="utf-8", newline="") as f:
            ...
            for doc in LidlProductItem.objects:  
                ...
                doc["site_shown_uom"] = doc["netcontent"]
                doc["instock"] = self.format_instock(doc)
                doc["currency"] = ""   # Force empty as requested
                doc["competitor_name"] = ""
                row = []
                for h in CSV_HEADERS:
                    value = doc.get(h, "")
                    if h in ["product_unique_key", "instock"]:
